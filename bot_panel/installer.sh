...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/KhaiVpn767/v4/main/bot_panel/installer.sh > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/KhaiVpn767/v4/main/bot_panel/installer.sh | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/KhaiVpn767/v4/main/bot_panel/installer.sh | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
[[ -d /etc/.maAsiss/info-user-xray ]] || mkdir /etc/.maAsiss/info-user-xray
...
dircreate
        ...
        echo -e "KHAIVPN Bot Panel Installer
        "
        [[ ! -f /root/ResBotAuth ]] && {
        ...
        echo -ne "Username admin panel use '@' [Ex: @khaivpn] : "
        read admin_pnl
        [[ -z $admin_pnl ]] && admin_pnl="@khaivpn"
        echo ""
        echo -ne "Limit trial for reseller create user trial [default:1] : "
        read limit_pnl
        [[ -z $limit_pnl ]] && limit_pnl="1"
        echo ""
        echo -ne "Your name store [dafult: KHAIVPN-STORE] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="KHAIVPN-STORE"
        echo ""
...
EOF
        ...
        fun_bot1() {
            [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
				wget -qO- botpanel.samproject.tech:81/BotAPI.sh >/etc/.maAsiss/.Shellbtsss
			}
